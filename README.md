...
[PR #4](https://github.com/Piyush-Piyush/Warden/pull/4) touches the higher-stakes code directly: `rollback_deploy` and `restart_service`, the two destructive, approval-gated tools, previously accepted empty strings for `project`, `service`, `target_commit_sha`, and `environment`, meaning a malformed or hallucinated tool call could reach a destructive handler before ever being caught. The fix adds minimum-length validation at the MCP input boundary plus regression tests. Qodo again reported zero bugs and zero violations, and its assessment specifically weighed schema-level validation against a shared validator or handler-level checks, and agreed the schema-level constraint was correct for this small, fixed set of fields. Same pattern as PR #3: no defect to dismiss, review passed clean, merged as reviewed.
